...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t            WELLCOME : [green] ADRIYAN-KING ENJOY PUBLIC TOOL[purple] '))
...
def dump_massal():
	...
	try:
		print('')
		print(f'{green}=>TOTAL ID★{green}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{u}')
...
def setting():
	...
	if hu in ['1','01']:
		...
		print('=>TRY AGAIN')
		exit()
	cetak(nel('\t[green]             • METHODE LOGIN • [/green]'))
	print('') 
	cetak(nel('[green purple]🤔➣1. MOBILE\n🙄➣2. MBASIC\n🤨➣3. TOUCH\n🤒➣4. MTOUCH [green purple]')) 
	print('')
	hc = input('>> CHOOSE : ')
	if hc in ['1','01']:
...
#   elif _jembot_ in ['y','Y']:
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[purple]•[green]] ADD PASWORD MXM 6 WORDS\n[[purple]•[green]] EXIMPLE :[green] 556677,786786,123456,@@@,1234,123,name+123[purple] '))
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	cetak(nel('[green]               WAIT FOR NAW ✌️ [green]')) 
	print(f'                    {m}A {k}D {h}R {u}I {b}Y{u}A{u}N 😇 {b}K {u}I {h} N{k} G{m} {b}')
	print('')
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crack,idf,pwv)
	print('')
	cetak(nel('\t[purple]=>[green] THANK YOU SO MUCH [purple] <<[yellow] '))
	print(f'[{h}•{u}]{h} OK : {h}%s '%(ok))
	print(f'{k}[{k}•{h}]{k} CP : {k}%s{u} '%(cp))
	print('')
	print('{k}=>Again Run  ( Y/t ) ? ')
	woi = input('=>Pilih : ')
	if woi in ['y','Y']:
		back()
	else:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}ADRIYAN-KING-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
